...
public:
  ...
  uint64_t memBytes() const { uint64_t bytes=0; for(const auto& i:mem) bytes+=i.first.size()+i.second.value.size()+24; return bytes; }
  uint64_t memtableLimit() const { try { auto mb=std::stoull(configValue("MEMTABLE_SIZE_MB")); return std::max<uint64_t>(1,mb)*1024*1024; } catch(...) { return 1ull*1024*1024; } }
  void flush(){ if(mem.empty()) return; std::string name="table-"+std::to_string(++sstables)+".sst",out=file(name); std::ofstream f(out); for(const auto& i:mem){ f<<i.first<<'\t'<<i.second.value<<'\t'<<i.second.seq<<'\n'; internalBytes+=i.first.size()+i.second.value.size()+24; } f.close(); tableFiles.push_back(name); persistTables(); mem.clear(); if(s3.enabled()){std::ofstream(file("wal.log"),std::ios::trunc);s3.upload("lsm/wal.log",file("wal.log"));}else std::ofstream(file("wal.log"),std::ios::trunc); walRecords=0; }
  void put(const std::string&k,const std::string&v){append("PUT",k,v);mem[k]=Record(v,false,seq);logicalBytes+=k.size()+v.size();if(memBytes()>=memtableLimit())flush();}
  void del(const std::string&k){append("DEL",k,"");mem[k]=Record("",true,seq);logicalBytes+=k.size();if(memBytes()>=memtableLimit())flush();}
  bool get(const std::string&k,std::string&v){++readOps;auto i=mem.find(k);if(i!=mem.end()){if(i->second.tombstone){++readMisses;return false;}v=i->second.value;return true;}for(auto t=tableFiles.rbegin();t!=tableFiles.rend();++t){std::ifstream f(file(*t));std::string line,key,value,version;while(std::getline(f,line)){std::stringstream row(line);if(!std::getline(row,key,'\t')||!std::getline(row,value,'\t')||!std::getline(row,version))continue;if(key==k){if(value.empty()){++readMisses;return false;}v=value;return true;}}}++readMisses;return false;}
  ...
  void loadTables(){for(const auto& n:tableFiles){std::ifstream f(file(n));std::string line,key,value,version;while(std::getline(f,line)){std::stringstream row(line);if(!std::getline(row,key,'\t')||!std::getline(row,value,'\t')||!std::getline(row,version))continue;uint64_t nseq=0;try{nseq=std::stoull(version);}catch(...){continue;}seq=std::max(seq,nseq);mem[key]=Record(value,value.empty(),nseq);}}}
  void crashRecover(){mem.clear();seq=0;loadTables();replay();}
...
static void handle(socket_t s,Engine& e){std::string req;char b[4096];int n;size_t headerEnd=std::string::npos;while((n=recv(s,b,sizeof(b),0))>0){req.append(b,n);headerEnd=req.find("\r\n\r\n");if(headerEnd!=std::string::npos)break;}auto line=req.substr(0,req.find("\r\n"));std::istringstream ls(line);std::string method,url,proto;ls>>method>>url>>proto;std::string body;size_t bodyStart=headerEnd==std::string::npos?req.size():headerEnd+4;size_t contentLength=0;if(headerEnd!=std::string::npos){auto headers=req.substr(0,headerEnd);auto marker=headers.find("Content-Length:");if(marker==std::string::npos)marker=headers.find("content-length:");if(marker!=std::string::npos){marker+=15;while(marker<headers.size()&&(headers[marker]==' '||headers[marker]=='\t'))++marker;try{contentLength=std::stoull(headers.substr(marker));}catch(...){contentLength=0;}}while(req.size()-bodyStart<contentLength&&(n=recv(s,b,sizeof(b),0))>0)req.append(b,n);body=req.substr(bodyStart,contentLength);}std::string out="{}";int code=200;
